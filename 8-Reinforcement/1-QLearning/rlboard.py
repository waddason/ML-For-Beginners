...
class Board:
    ...
    def randomize(
        ...
        for _ in range(num_water):
            x, y = self.random_pos()
            # print(f"Water pond centerd at {x, y}")
    ...
    def is_valid(self, pos):
        return (
            pos[0] >= 0 and pos[0] < self.width and pos[1] >= 0 and pos[1] < self.height
        )
    ...
    def image(self, Q=None):
        ...
        # Draw cells by replacing the corresponding rectangle with the image on the 3 RGB layers
        for x in range(self.width):
            for y in range(self.height):
                if (x, y) == self.human:
                    ov = self.pics["human"]
                    img[
                        self.size * y + 2 : self.size * y + ov.shape[0] + 2,
                        self.size * x + 2 : self.size * x + 2 + ov.shape[1],
                        :,
                    ] = np.minimum(ov, 1.0)
                    continue
                ...
                if self.matrix[x, y] == Board.Cell.empty and Q is not None:
                    ...
                    for i, (ddx, ddy) in enumerate([(-1, 0), (1, 0), (0, -1), (0, 1)]):
                        dx += ddx * p[i]
                        dy += ddy * p[i]
                        square_line = draw_line(dx, dy, self.size)
                        img[
                            self.size * y + 2 : self.size * y
                            + square_line.shape[0]
                            + 2,
                            self.size * x + 2 : self.size * x
                            + 2
                            + square_line.shape[1],
                            :,
                        ] = square_line

        # Draw grid by changing a full line to the grid color
        for i in range(self.height + 1):
            img[:, i * self.size] = [0.3, 0.3, 0.3]
    ...
    def plot(self, Q=None):
        ...
        plt.imshow(self.image(Q), interpolation="lanczos")

    def saveimage(self, filename, Q=None):
        cv2.imwrite(filename, 255 * self.image(Q)[..., ::-1])  # convert to BGR
    ...
    def text_display(self):
        print(self.matrix.astype(int).transpose())
